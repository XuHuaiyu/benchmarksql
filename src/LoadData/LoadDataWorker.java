...
public class LoadDataWorker implements Runnable {
	...
	LoadDataWorker(int worker, String csvNull, jTPCCRandom rnd, String tableName) {
		this.worker = worker;
	...
	public void run() {
		...
		try {
			while ((job = LoadData.getNextJob()) >= 0) {
				if (job == 0) {
					if (this.tableName.contains("all") || this.tableName.contains("customer")){
	...
	private void loadWarehouse(int w_id)
			throws SQLException, IOException {
		if (this.tableName.contains("all") || this.tableName.contains("stock")) {
			/*
		...
		for (int d_id = 1; d_id <= 10; d_id++) {
			...
			if (this.tableName.contains("all") || this.tableName.contains("order")) {
				/*
